import { resolveAsset } from './PreviewApp';
...
export function PreviewContent({ url }: { url?: string }) {
    ...
    if (!src) {
        return (
            <div style={{ color: 'var(--fg-dim)', fontSize: '13px' }}>
                No résumé configured. Add <code>resume.pdf</code> to <code>public/</code> and set{' '}
                <code>meta.resumeUrl</code> in your data.json.
            </div>
        );
    }
    ...
    return (
        ...
        >
            <div style={{ color: 'var(--fg-dim)', fontSize: '13px', padding: '8px 0' }}>
                This browser can&rsquo;t display the PDF inline.{' '}
                <a href={src} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--info)' }}>
