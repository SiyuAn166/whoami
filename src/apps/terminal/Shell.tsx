...
import './terminal.css';
import { USER, HOST, HINTS, COARSE_POINTER } from './shell/constants';
...
export function Shell({ data, theme, setTheme }: ShellProps) {
    ...
    const pushCmd = (prompt: string, cmd: string, output: React.ReactNode | null) =>
        setLines(l => [...l, { id: nextId(), prompt, cmd, output }]);

    // ── Command interpreter ───────────────────────────────────────────────
    const execute = (raw: string) =>
        runCommand(raw, { fs, cwd, theme, data, history, setCwd, setLines, setTheme, setMatrixOn, pushCmd });
