...
export function Desktop() {
  ...
  return <DesktopReady data={data} theme={theme} toggleTheme={toggleTheme} setTheme={setTheme} />;
}
 ...
 */
function DesktopReady({ data, theme, toggleTheme, setTheme }: DesktopReadyProps) {
  const wm = useWindowManager({ data, theme, setTheme });
  const focusedApp = wm.instances.find(w => w.id === wm.focusedId);
  const menuBarAppName = (focusedApp && getApp(focusedApp.appId)?.name) ??
    'Finder';
  return (
    <div
      className="mac-desktop"
      style={data.meta.wallpaper ?
        { backgroundImage: `url(${data.meta.wallpaper})`, backgroundSize: 'cover', backgroundPosition: 'center' } : undefined}
    >
      ...
      <WidgetLayer data={data} theme={theme} setTheme={setTheme} openApp={wm.openApp} />
      {wm.render()}
