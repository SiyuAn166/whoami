...
const TIER_H: Record<WidgetSize, number> = { small: 190, medium: 210, wide: 150, large: 380 };
...
export function WidgetLayer(props: WidgetLayerProps) {
  ...
  const drag = useRef<{ id: string; startX: number; startY: number; originX: number; originY: number; moved: boolean; pointerId: number } | null>(null);
  ...
  useLayoutEffect(() => {
    ...
    setPositions(prev => {
      ...
      for (const w of visible) {
        ...
        const p = clampXY(width, sizeOf(w.id).h, x, prev[w.id]?.y ?? MARGIN.top);
        if (prev[w.id]?.x !== p.x) { next[w.id] = { ...next[w.id], ...p }; changed = true; }
      }
  ...
  const sizeOf = useCallback((id: string): { w: number; h: number } => {
    const el = layerRef.current?.querySelector<HTMLElement>(`[data-wid="${id}"]`);
    return { w: el?.offsetWidth ?? 200, h: el?.offsetHeight ?? 200 };
  }, []);

  const clampXY = useCallback((w: number, h: number, x: number, y: number): Pos => {
  ...
  const clamp = useCallback((id: string, x: number, y: number): Pos => {
    const { w, h } = sizeOf(id);
   ...
   * it was dropped.
   */
  const resolveCollision = useCallback((id: string, desired: Pos, all: PosMap): Pos => {
    const { w, h } = sizeOf(id);
    const others: Rect[] = [];
    ...
    if (free(desired.x, desired.y)) return clampXY(w, h, desired.x, desired.y);

    // Expanding-ring search on the grid around the desired cell.
    const maxRing = Math.ceil(Math.max(window.innerWidth, window.innerHeight) / GRID);
  ...
  const onPointerDown = useCallback((e: ReactPointerEvent<HTMLDivElement>, id: string) => {
    ...
    drag.current = {
      ...
      originX: cur.x, originY: cur.y, moved: false, pointerId: e.pointerId,
    };
