...
export function WidgetGallery({
  ...
  return (
    <>
      {/* click the empty desktop (scrim) to slide out. DesktopClickMenu.tsx
          imports this same module and uses styles.widgetgalleryScrim/Panel
          in its .closest() selector. */}
      <div
        className={`${styles.widgetgalleryScrim}${open ? " " + styles.isOpen : ""}`}
        onClick={close}
      />
      <div
        className={`${styles.widgetgalleryPanel}${open ? " " + styles.isOpen : ""}`}
        role="dialog"
        aria-label="Add widgets"
      >
        <div className={styles.widgetgallery__grip} />

        <div className={styles.widgetgallery__head}>
          <div className={styles.widgetgallery__title}>Widgets</div>
          <div className={styles.widgetgallery__pills}>
            <button
              className={`${styles.widgetgallery__pill}${group === "all" ? " " + styles.isActive : ""}`}
            ...
            {variants.map((v) => (
              <button
                key={v}
                className={`${styles.widgetgallery__pill}${group === v ? " " + styles.isActive : ""}`}
                onClick={() => setGroup(v)}
              >
                {VARIANT_LABEL[v]}
              </button>
        ...
        <div className={styles.widgetgallery__grid}>
          {list.map((w) => {
            const active = activeIds.includes(w.id);
            return (
              <div
                className={styles.widgetgallery__card}
                key={w.id}
                onClick={() => {
                  onAdd(w.id);
                  close();
              ...
              >
                <div className={styles.widgetgallery__tile} data-size={w.size}>
                  {active && (
                    <span className={styles.widgetgallery__badge}>
                      ON DESKTOP
                    </span>
                  )}
                  <div className={styles.widgetgallery__preview} aria-hidden>
                    <div className={styles.widgetgalleryPreviewScale}>
                  ...
                  <div className={styles.widgetgallery__plus}>
                    {/* plus sign */}
                    <svg
                      width="14"
                      height="14"
                ...
                <div className={styles.widgetgallery__caption}>
                  <b>{w.title ?? w.id}</b>
                  <span>{w.size}</span>
                </div>
              </div>
          ...
          {list.length === 0 && (
            <div className={styles.widgetgallery__empty}>
              No widgets match “{q}”.
            </div>
          )}
        </div>

        <div className={styles.widgetgallery__foot}>
          <button className={styles.widgetgallery__done} onClick={close}>
            Done
          </button>
        </div>
      </div>
